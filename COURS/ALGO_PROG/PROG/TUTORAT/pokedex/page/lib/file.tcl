...
proc vTcl:save2 {file} {
    ...
    if {$vTcl(tops) == ""} {
        ::vTcl::MessageBox -title "Error saving project" \
            -message "You are trying to save a void project; there is no top level widget" \
                  -icon error \
                  -type ok
        return -1
    }
    if {!$vTcl(change) && $vTcl(top_geometry) == [wm geometry $top] &&
        [file exists $file]} {
        return -1
    }
