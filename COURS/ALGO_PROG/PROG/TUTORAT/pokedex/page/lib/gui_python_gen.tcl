...
# Copyright (C) 2013-2017 Donald Rozenberg
#
#
...
#    set vTcl(py_window) $window
    ...
    if {$vTcl(py_title) == ""} {
        ...
        set  vTcl(py_classname) [string map { " " _ } $alias ]
    } else {
        set  vTcl(py_classname) [string map { " " _ } $vTcl(py_title) ]
    }

    #.vTcl
    #.vTcl.gui_console.f3.t delete 1.0 end
    #.vTcl.gui_console.f5.t delete 1.0 end
    #.vTcl.gui_console.pane.text1.text1 subwidget text delete 1.0 end
    #.vTcl.gui_console.pane.text2.text2 subwidget text delete 1.0 end

    # If there is something already in the the source window and it
    # was code for the support module and it was not saved see if user
    # wants to save it. 11-4-14

    # I think I can dispense with the following because I am no longer
    # sharing one console window.  1/8/16
    # if {[info exists vTcl(python_module)] && $vTcl(python_module) == "Support"} {
    #     set base .vTcl.gui_console
    #     vTcl:check_upon_close_button  $base
    # }
    vTcl:Window.vTcl.py_console "gui"
    update
    $vTcl(gui_source_window) delete 1.0 end
    $vTcl(gui_output_window) delete 1.0 end
    vTcl:save             ;# This saves the tcl version.
    # If we have a popup menu, generate that code separately.
...
#    set vTcl(popup_binding) \
# "
#     if (root.tk.call('tk', 'windowingsystem')=='aqua'):
#        root.bind('<Button-2>', self.popup)
#        root.bind('<Control-1>', self.popup)
#     else:
...
#     # This is meant tosupport a feature that I have not decided to release.
#     # Determine relative position.
#     set prop_geom $vTcl(pr,proportional_geometry)
#     if {[info exists ::widgets::${window}::prop,geom]} {
#         set prop_geom [vTcl:at ::widgets::${window}::prop,geom]
#     }
#     #set screen_geom [wm maxsize .]
#     set s_w [winfo screenwidth .]
#     set s_h [winfo screenheight .]

#     foreach {w h x y} $geom_split {}
#     set rel_w [expr $w / $s_w.0]
#     set rel_h [expr $h / $s_h.0]
#     set rel_x [expr $x / $s_w.0]
#     set rel_y [expr $y / $s_h.0]
#     if {$prop_geom} {
#         append geom_output \
# "
...
#             append geom_output \
# "
#     pos_x = int(round($rel_x * root_width))
#     pos_y = int(round($rel_y * root_height))
#     width = int(round($rel_w * root_width))
#     height = int(round($rel_h * root_height))
#     geom = '%sx%s+%s+%s' % (width, height, pos_x,  pos_y)"
#         } else {
#             append geom_output \
# "
#     width = int(round($rel_w * root_width))
#     height = int(round($rel_h * root_height))
#     geom = '%sx%s' % (width, height)"
#         }
#     } else {
        if {!$dflt_origin} {
...
proc vTcl:Window.vTcl.py_console {type} {
    # Builds the  Python Console
    global vTcl
    set prefix $type
    set base .vTcl.${prefix}_console
    if {[winfo exists $base]} {
        wm deiconify $base; raise .vTcl.${prefix}_console
        return
    }
    set top $base
    ###################
...
-anchor se
    ...
    catch {wm geometry .vTcl.${prefix}_console \
               $vTcl(geometry,.vTcl.${prefix}_console)}

    set old_geometry [wm geometry .vTcl.${prefix}_console]
    ...
    # save label
    if {$prefix == "gui"} {
        bind $vTcl(gui_code_window) <KeyRelease> {
            set vTcl(gui_save_warning) "Unsaved changes"
        }
    } else {
        bind $vTcl(supp_code_window) <KeyRelease> {
...
#                     }
            ...
            justify {
                ...
                if {$v == "READONLY"} {
                    # This is a hack because of a tkinter bug which
                    # does not define the variable READONLY. THis
                    # seems like the quickest easist fix which will
                    # still be OK when and if they fix the bug.
                    append vTcl(py_initfunc) \
                        "$vTcl(tab2)READONLY = 'readonly'\n"
                }
                append vTcl(py_initfunc) \
                    "$vTcl(tab2)$name.configure($sub=$v)\n"
            }
...
proc vTcl:progressbar_placement {} {
...
proc vTcl:python_source_widget {target prefix parentframe} {
    ...
    if {$gpc == "TNotebook"} {
        foreach t [$grand_parent_window tabs] {
            if {[winfo ismapped $t]} {
                set parent_geometry [winfo geometry $t]
                break
            }
        }
    } else {
    ...
    # # is in notebook. I don't think we need this any longer. 4/3/17
...
#NEEDS WORK for activebackground.
                    append vTcl(py_initfunc) \
                 "$vTcl(tab2)self.$readable_name = Menu(top$f_s$bg_s$fg_s)\n"
                    append vTcl(py_initfunc) \
    ...
    for {set i 0} {$i<=$entries} {incr i} {
        ...
        switch $type {
            cascade {
                ...
                if {$vTcl(in_popup)} {
                    #set parent_alias $widget(rev,$target)
                    #append vTcl(py_initfunc) \
                      "$vTcl(tab2)self.$lab_name = Menu($parent_alias$tearoff)\n"
                    append vTcl(py_initfunc) \
                      "$vTcl(tab2)self.$lab_name = Menu($widname$tearoff)\n"
                } else {
                    append vTcl(py_initfunc) \
                        "$vTcl(tab2)self.$lab_name = Menu(top$tearoff)\n"
                }
...
"
    ...
    root.mainloop()"
        ...
        append start_gui \
"    $vTcl(import_name).init(root, top)
    root.mainloop()
"
    }
...
"
    ...
    return (w, top)"
        ...
        append start_gui \
"
    $vTcl(import_name).init(w, top, *args, **kwargs)
    return (w, top)
"
    }
...
proc vTcl:load_console {prefix file} {
    global vTcl
    # If the consol already exists then return
    if {[winfo exists .vTcl.${prefix}_console]} {
        Window show .vTcl.${prefix}_console
        if {$prefix == "gui"}  {
            set question "GUI Console exists do you want to overwrite it?"
        } else {
            set question "Support Console exists do you want to overwrite it?"
            set choice [tk_dialog .foo "Warning" $question \
                         question 0 "No" "Overwrite"]
            if {$choice == 0} {
                return
            }
        }
    } else {
        vTcl:Window.vTcl.py_console $prefix
    }
    update
    ...
    $vTcl(${prefix}_source_window) insert end $contents
