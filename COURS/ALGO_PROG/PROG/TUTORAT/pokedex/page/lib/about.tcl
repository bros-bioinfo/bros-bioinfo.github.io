...
Copyright (C) Donald Rozenberg 2001-2016

Licensed under GNU General Public License
version 2

Documentation can be found in the \"<page_dir>\docs\" subdirectory.
...
proc vTcl:open_help_in_browser { } {
    ...
    if {$tcl_platform(platform) == "windows"} {
        ...
        if {[catch {set browser $::env(BROWSER)}]} {
            ::vTcl::MessageBox -icon info -message \
                "You need to set the envirnmental variable BROWSER to something like:\n\"firefox\"" \
                -title "Missing Variable" -type ok
            return 0
        }
        set browser $::env(BROWSER)
        if {$browser == "firefox"} {
