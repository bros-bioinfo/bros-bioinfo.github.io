...
//get date associé
for (i=0; i<nbfichiers; i=i+1){
  var relativepath=strarray[i]
  relativepath=relativepath.replace(/<a href="http:\/\/bros-bioinfo.github.io/gm, '');
  relativepath=relativepath.replace(/"><\/a>/gm, '');
  relativepath=relativepath.replace(/ /gm,"");
  urldate="https://api.github.com/repos/bros-bioinfo/bros-bioinfo.github.io/commits?per_page=1&master&path="+relativepath+".md"
  console.log(urldate);
  $.get(urldate, function(datadate) {
    date = String(datadate);
    //date = String(date).replace(/^((?!md|pdf).)*$/gm, "");
    console.log(date);
  }, 'text');
  ...
  while (n < nbfichiers) {
    ...
    while (i < tab.length ) {
      ...
      url = url.replace(/╶─ ALGO ╶─/,' ╶─ <div style="color:white" class="algo"><i class="fa fa-folder-open" aria-hidden="true"></i> ALGO </div>╶─');
